...
describe('PixelData Inversion', () => {
  ...
  it('correctly inverts a Red pixel to Cyan via the 32-bit view', () => {
    ...
    expect(pixels.data[0]).toBe(CYAN)
  ...
  it('handles bitwise shift logic for data32 length', () => {
    ...
    expect(pixels.data.length).toBe(4)
  })
  ...
  it('inverts only a specified sub-rectangle', () => {
    ...
    expect(pixels.data[0]).toBe(RED) // top-left
    expect(pixels.data[1]).toBe(RED) // top-right
    expect(pixels.data[2]).toBe(RED) // bottom-left
    expect(pixels.data[3]).toBe(CYAN) // bottom-right
  })
  ...
  it('correctly applies a binary mask', () => {
    ...
    expect(pixels.data[0]).toBe(CYAN) // (0,0) - mask=1
    expect(pixels.data[1]).toBe(RED)  // (1,0) - mask=0
    expect(pixels.data[2]).toBe(RED)  // (0,1) - mask=0
    expect(pixels.data[3]).toBe(CYAN) // (1,1) - mask=1
  })
  ...
  it('correctly applies an inverted binary mask', () => {
    ...
    expect(pixels.data[0]).toBe(RED)  // (0,0) - mask=1, inverted
    expect(pixels.data[1]).toBe(CYAN) // (1,0) - mask=0, inverted
    expect(pixels.data[2]).toBe(CYAN) // (0,1) - mask=0, inverted
    expect(pixels.data[3]).toBe(RED)  // (1,1) - mask=1, inverted
  })
  ...
  it('clips the operation if the rectangle is out of bounds', () => {
    ...
    expect(pixels.data[0]).toBe(RED)
    expect(pixels.data[1]).toBe(RED)
    expect(pixels.data[2]).toBe(RED)
    expect(pixels.data[3]).toBe(CYAN) // Only (1,1) should be inverted
  })
  ...
  it('handles negative coordinates by clipping', () => {
    ...
    expect(pixels.data[0]).toBe(CYAN) // Only (0,0) should be inverted
    expect(pixels.data[1]).toBe(RED)
    expect(pixels.data[2]).toBe(RED)
    expect(pixels.data[3]).toBe(RED)
  })

  it('does nothing if the rectangle is entirely out of bounds', () => {
    const pixels = makeTestPixelData(2, 2, RED)
    const originalData = new Uint32Array(pixels.data)

    invertPixelData(pixels, { x: 10, y: 10, w: 2, h: 2 })

    expect(pixels.data).toEqual(originalData)
  })
})
