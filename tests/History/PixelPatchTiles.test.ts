...
describe('PixelPatchTiles', () => {
  ...
  describe('applyPatchTiles', () => {
    ...
    it('should apply a single tile to the target', () => {
      ...
      for (let y = 4; y < 8; y++) {
        for (let x = 4; x < 8; x++) {
          ...
          expect(targetPixelData.data[idx]).toBe(0xFFFFFFFF)
        }
      }

      // Check a pixel outside the tile to ensure it wasn't modified
      expect(targetPixelData.data[0]).toBe(0)
    })

    it('should apply multiple tiles', () => {
      const tile1 = makePixelTile(0, 0, 0, tileSize, tileArea)
      tile1.data.fill(0xFF0000FF) // Red

      const tile2 = makePixelTile(1, 1, 0, tileSize, tileArea)
      tile2.data.fill(0x00FF00FF) // Green

      applyPatchTiles(targetPixelData, [tile1, tile2], tileSize)

      // Check tile1 area (0,0) -> (3,3)
      expect(targetPixelData.data[0]).toBe(0xFF0000FF)
      expect(targetPixelData.data[3 * targetPixelData.w + 3]).toBe(0xFF0000FF)

      // Check tile2 area (4,0) -> (7,3)
      expect(targetPixelData.data[4]).toBe(0x00FF00FF)
      expect(targetPixelData.data[3 * targetPixelData.w + 7]).toBe(0x00FF00FF)
    })
    ...
    it('should correctly clamp tiles at the image boundaries', () => {
      ...
      // [8,8] = 1, [8,9] = 2
      // [9,8] = 5, [9,9] = 6
      expect(targetPixelData.data[8 * 10 + 8]).toBe(1)
      expect(targetPixelData.data[8 * 10 + 9]).toBe(2)
      expect(targetPixelData.data[9 * 10 + 8]).toBe(5)
      expect(targetPixelData.data[9 * 10 + 9]).toBe(6)

      // Check that pixels outside this 2x2 area are not touched
      expect(targetPixelData.data[7 * 10 + 8]).toBe(0) // Above
      expect(targetPixelData.data[8 * 10 + 7]).toBe(0) // Left
    })
    ...
    it('should do nothing if a tile is outside the image bounds', () => {
      ...
      targetPixelData.data.forEach(pixel => expect(pixel).toBe(0))
    })

    it('should handle an empty array of tiles', () => {
      const originalData = new Uint32Array(targetPixelData.data)
      applyPatchTiles(targetPixelData, [], tileSize)
      expect(targetPixelData.data).toEqual(originalData)
    })
    ...
    it('should skip undefined or null tiles in the array', () => {
      ...
      expect(targetPixelData.data[0]).toBe(0xFFFFFFFF)
    })
  })
})
